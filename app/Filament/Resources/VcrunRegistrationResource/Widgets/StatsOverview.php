...
class StatsOverview extends BaseWidget
{
    protected function getCards(): array
    {
        ...
        $cards = [
            Card::make('Total Expected Amount 2022-present', 'Ksh ' . number_format($expectedAmount, 2, '.', ','))
                ->description('100% Expected')
                ->descriptionIcon('heroicon-s-sparkles')
                ->chart([0, 20, 40, 60, 80, 100])
                ->color('success'),

            Card::make('Total Amount Paid 2022-present', 'Ksh ' . number_format($paidRegistrations, 2, '.', ','))
